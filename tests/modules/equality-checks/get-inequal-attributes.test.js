...
const outputSchema = z.string().array()
...
describe("Exclusions", () => {
    test("Top-level exclusion", () => {
        // Top-level exclusion does not prevent top-level checks
        ...
        // But disables recursive checks
        a = {x: {y1: 1}}, b = {x: 2}, exclusions = [""];
        expect(getInequalAttributes(a, b, exclusions)).toHaveInequalAttributes([]);
    });
    ...
    test("Asterisks in sets", () => {
        // Asterisks in sets exclude comparison of their items, if lengths of sets are equal
        let a = {x: new Set([
            {y: 1, z: 1},
            {y: 2, z: 2}
        ])},
