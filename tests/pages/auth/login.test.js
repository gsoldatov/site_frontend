...
import { 
    mockAuthLoginUnauthorizedMessage,
    mockAuthLoginUnauthorized,
    mockAuthLoginTooManyRequests
} from "../../_scenarios/backend-responses/auth";
...
test("Validation error display", async () => {
    // Render login page
...
test("Fetch error display & form disabling during fetch", async () => {
    // Render login page
    const { store } = createTestStore({ addAdminToken: false });
    let { container } = renderWithWrappers(<App />, {
        route: "/auth/login", store
...
test("Wrong credentials", async () => {
    ...
    // Set custom /auth/login response
    mockAuthLoginUnauthorized();
    ...
    // Wait for error message to appear
    await authLoginActions.waitForFormError(mockAuthLoginUnauthorizedMessage);
});
...
test("Too many requests", async () => {
    ...
    // Wait for error message to appear
    await authLoginActions.waitForFormError("Too many login attempts performed. Retry after");
});
...
test("Correct login and form error reset", async () => {
    ...
    expect(
        ...
        - (new Date(authState.access_token_expiration_time)).getTime()
                            ...
                            // (other fetches overwrite the timestamp in the state)
});
